 ...
 */
@WebServlet("/Sl_FotoServicio")
public class Sl_FotoServicio extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Sl_FotoServicio() {
        super();
...
//					if(formatos.contains(uploaded.getContentType()) && size <= 102400)
					if(formatos.contains(uploaded.getContentType()))
					{
						...
						rutaFichero = "fotoServicio_"+idservicio+".jpg";
						path = "C:\\payara5\\glassfish\\PWACP-fotos_servicios\\";
						...
						String url = "PWACP-fotos_servicios/"+rutaFichero;
						
						if(dtser.guardarFotoServicio(Integer.parseInt(idservicio),url))
						{
							response.sendRedirect("tblServicios.jsp?msj=7");
						}
						else
						{
							response.sendRedirect("tblServicios.jsp?msj=8");
						}
					}
					else
					{
						System.out.println("SERVIDOR: VERIFIQUE QUE EL ARCHIVO CUMPLA CON LAS ESPECIFICACIONES REQUERIDAS!!!");
						response.sendRedirect("tblServicios.jsp?msj=9");						
					}
				}
