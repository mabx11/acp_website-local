...
//		doGet(request, response);
		...
		case 1:
			try {
				...
				if(us.getUsuarioID()!=0){
					dtem.recuperaPwd(us.getUsuario(), us.getEmail());
					response.sendRedirect("recuperacionCuenta.jsp?msj=1");
				}
				else {
					response.sendRedirect("recuperacionCuenta.jsp?msj=2");
				}
		...
		case 2:
			try {
				...
				if(dtu.updatePwd(us, newPwd)){
					response.sendRedirect("inicioSesion.jsp?msj=1");
					//REENVIAMOS AL LOGIN INDICANDO QUE EL PROCESO DE ACTUALIZACION DE CONTRASEÑA FUE EXITOSO
				}
				else {
					response.sendRedirect("recuperacionCuenta.jsp?msj=3");
			...
			break;
		default:
			response.sendRedirect("recuperacionCuenta.jsp?msj=4");
