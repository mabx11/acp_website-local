...
//					if(formatos.contains(uploaded.getContentType()) && size <= 102400)
					if(formatos.contains(uploaded.getContentType()))
					{
						...
						rutaFichero = "fotoBanner_"+idhome+".jpg";
						path = "C:\\payara5\\glassfish\\PWACP-fotos_banner\\";
						...
						String url = "PWACP-fotos_banner/"+rutaFichero;
						
						if(dthm.guardarFotoBanner(Integer.parseInt(idhome),url))
						{
							response.sendRedirect("index.jsp?msj=1"+idhome+"&guardado=1");
						}
						else
						{
							response.sendRedirect("index.jsp?msj=1"+idhome+"&guardado=2");
						}
					}
					else
					{
						System.out.println("SERVIDOR: VERIFIQUE QUE EL ARCHIVO CUMPLA CON LAS ESPECIFICACIONES REQUERIDAS!!!");
						response.sendRedirect("index.jsp?msj=2"+idhome+"&guardado=3");						
					}
				}
